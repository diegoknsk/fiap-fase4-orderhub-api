...
/// <summary>
/// UseCase para atualizar produto no pedido
/// </summary>
public class UpdateProductInOrderUseCase
{
    ...
    public UpdateProductInOrderUseCase(
        IOrderDataSource orderDataSource,
        IProductDataSource productDataSource,
        UpdateProductInOrderPresenter presenter)
    {
    ...
    public async Task<UpdateProductInOrderResponse> ExecuteAsync(UpdateProductInOrderInputModel input)
    {
        // Validar quantidade
        if (input.Quantity <= 0)
        ...
        // Buscar OrderedProduct dentro do Order.OrderedProducts
        ...
        // Buscar produto para obter informações atualizadas
        var productDto = await _productDataSource.GetByIdAsync(orderedProduct.ProductId);
        if (productDto == null)
        ...
        // Atualizar OrderedProduct usando métodos de domínio
        orderedProduct.Product = product;
        orderedProduct.SetQuantity(input.Quantity);
        orderedProduct.SetObservation(input.Observation);

        // Atualizar ingredientes customizados
        if (input.CustomIngredients.Any())
        {
            orderedProduct.CustomIngredients.Clear();
            ...
            // Recalcular FinalPrice após atualizar ingredientes
            orderedProduct.CalculateFinalPrice();
        }

        // Recalcular TotalPrice do Order usando método de domínio
    ...
    private UpdateProductInOrderOutputModel AdaptToOutputModel(Order order, OrderedProduct orderedProduct)
    {
        return new UpdateProductInOrderOutputModel
        {
            OrderId = order.Id,
            OrderedProductId = orderedProduct.Id,
            TotalPrice = order.TotalPrice
